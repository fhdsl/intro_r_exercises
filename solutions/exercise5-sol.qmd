...
```{r}
simple_df$sum_measurements = simple_df$measurement1 + simple_df$measurement2 + simple_df$measurement3

simple_df = mutate(simple_df, sum_measurements = measurement1 + measurement2 + measurement3)
```
...
```{r}
simple_case = filter(simple_df, case_control == "case")

mean(simple_case$sum_measurements)

simple_control = filter(simple_df, case_control == "control")

mean(simple_control$sum_measurements)
```
...
```{r}
metadata$medicaid_eligible = medicaid_eligible(metadata$Age)
#or
metadata = mutate(metadata, medicaid_eligible = medicaid_eligible(Age))
```
...
simple_df
simple2_df
inner_df = full_join(simple_df, simple2_df, by = "id")
dim(inner_df)
View(inner_df)
```
...
full_join(full_join(metadata, expression, by = "ModelID"), mutation, by = "ModelID")
...
```{r}
analysis = filter(Age > 18 & Age < 45) %>% select(Age, OncotreeLineage, PrimaryOrMetastasis)
...
analysis = filter(Age > 18 & Age < 45) %>% select(Age, OncotreeLineage, PrimaryOrMetastasis) %>% mutate(medicaid = medicaid_eligible(Age))

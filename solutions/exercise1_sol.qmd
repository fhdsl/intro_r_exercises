...
Explain in words how the R Console parse out this expression to give the resulting value. The `sqrt()` function computes the square root of a number, and `abs()` function computes the absolute value of a number. Try `?sqrt` and `?abs` for more information.
...
Now, modify the third element of this `chrNum` vector to be doubled. Also, modify `chrNum` so that chr 21 is the sum of chr 2, 3, and 4.
...
Try the following: access chromosomes 1 to 10. Instead of writing out 1 to 10 for your indexing vector, use the sequence `seq()` function: try `seq(1, 10)` to create your indexing vector. You could also use the operation `1:10` instead of `seq()` as an alternative:
